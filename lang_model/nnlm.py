...
train, val, test = torchtext.datasets.WikiText2.splits(path=".\\examples\\word_language_model\\data\\wikitext-2", train="train.txt", validation="valid.txt", test="valid.txt", text_field=TEXT)
TEXT.build_vocab(train, max_size=1000) if False else TEXT.build_vocab(train)
TEXT.vocab.load_vectors('glove.840B.300d', cache="C:\\Temp\\")
...
def string_to_batch(string):
    relevant_split = string.split()[-2:] # last two words, ignore ___
    ids = [(lambda word: TEXT.vocab.stoi[word])(word) for word in relevant_split]
    if use_cuda:
        return Variable(torch.LongTensor(ids)).cuda()
    else:
        return Variable(torch.LongTensor(ids))
def predict_sentence(string, model):
    string = string[:-4]
    x = string_to_batch(string)
    out_words = model.predict(x)
    return out_words
def write_kaggle(model, input_file = 'input.txt'):
        inputs = open(input_file, 'r').read().splitlines()
        outputs = [predict_sentence(sentence, model) for sentence in inputs]
        with open('nnlm_output.txt', 'w') as f:
            f.write('id,word')
            for idx, line in enumerate(outputs):
                f.write('\n')
...
trainer.train_model(model = model, num_epochs = 5)
pass